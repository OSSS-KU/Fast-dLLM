...
def main():
    ...
    data = []
    with open("outs/preds_poisson_gsm8k.json", "r") as f:
        for line in f:
    ...
    for d in data:
        ...
        prediction = tokenizer.batch_decode(out[0][:, input_ids.shape[1]:], skip_special_tokens=True)[0]
        if d["prediction"] != prediction:
            print("-" * 15, d["index"], "-" * 15)
            print_diff(d["prediction"], prediction)
