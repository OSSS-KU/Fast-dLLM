...
class LLaDAModelLM(PreTrainedModel):
    ...
    # ===== ORCA-style packed scheduling state =====
    def _ensure_orca_store(self):
        if not hasattr(self, "_orca_kv_store"):
            # { req_id -> [ per-layer (k, v) or None ] }
            from collections import defaultdict
            L = self.model.config.n_layers
            self._orca_kv_store = defaultdict(lambda: [None] * L)

    @torch.no_grad()
    def forward_batch(
        self,
        tokens_batch: torch.LongTensor,        # [ΣL] (concat input_ids) 또는 [1, ΣL]
        pos_batch: torch.LongTensor,           # [ΣL] absolute positions
        segments_or_rids: List,                # [(offset, length, rid), ...] 또는 rid_list
    ) -> Dict[str, Any]:
        """
        ORCA-style selective batching:
        - projection: concat 병렬
        - attention: 요청별 segment로 serial
        - FFN: 다시 병렬(cat→FFN→scatter)

        입력 형태 유연화:
        - segments_or_rids 가 (o,l,r) 튜플들의 리스트면 그대로 사용
        - 아니면 rid_list 로 간주하고 pos_batch의 연속구간을 스캔해 segments를 복원
        """
        # ---------- 입력 정규화 ----------
        if tokens_batch.dim() == 2 and tokens_batch.size(0) == 1:
            tokens_batch = tokens_batch.view(-1)                # [ΣL]
        elif tokens_batch.dim() != 1:
            raise ValueError(f"tokens_batch must be [ΣL] or [1, ΣL], got {tuple(tokens_batch.shape)}")

        if pos_batch.dim() == 2 and pos_batch.size(0) == 1:
            pos_batch = pos_batch.view(-1)                      # [ΣL]
        elif pos_batch.dim() != 1:
            raise ValueError(f"pos_batch must be [ΣL] or [1, ΣL], got {tuple(pos_batch.shape)}")

        S = int(tokens_batch.numel())
        if int(pos_batch.numel()) != S:
            raise ValueError(f"pos_batch length ({int(pos_batch.numel())}) must match tokens_batch ({S})")

        # segments_or_rids 파싱
        segments: List[Tuple[int, int, int]] = []
        if len(segments_or_rids) > 0 and isinstance(segments_or_rids[0], (tuple, list)) and len(segments_or_rids[0]) == 3:
            # 이미 (off, len, rid) 리스트로 들어온 경우
            segments = [(int(o), int(l), int(r)) for (o, l, r) in segments_or_rids if int(l) > 0]
        else:
            # rid_list 로 들어온 경우 → pos_batch의 연속구간을 찾아 offset/length 복원
            rid_list = [int(r) for r in segments_or_rids]
            # 연속(pos[i] == pos[i-1] + 1)이 깨지는 지점에서 구간 나누기
            runs: List[Tuple[int, int]] = []  # (offset, length)
            start = 0
            for i in range(1, S):
                if int(pos_batch[i].item()) != int(pos_batch[i-1].item()) + 1:
                    runs.append((start, i - start))
                    start = i
            runs.append((start, S - start))

            if len(runs) != len(rid_list):
                raise ValueError(
                    f"Number of runs inferred from pos_batch ({len(runs)}) != len(rid_list) ({len(rid_list)}). "
                    "Make sure each request contributes exactly one contiguous span per step."
                )
            segments = [(off, length, rid_list[i]) for i, (off, length) in enumerate(runs) if length > 0]

        if not segments:
            # 빈 배치
            return {
                "logits_cat": torch.empty((0, self.model.config.vocab_size),
                                        device=self.model.device,
                                        dtype=self.model.transformer.wte.weight.dtype),
                "updated_kv_handles": {},
            }

        # 안정적 처리 위해 offset 기준 정렬
        segments.sort(key=lambda x: x[0])

        # ---------- 공통 준비 ----------
        self._ensure_orca_store()
        cfg = self.model.config
        dev = self.model.device
        dtype_w = self.model.transformer.wte.weight.dtype

        # 0) 임베딩 (merge)
        x = self.model.transformer.wte(tokens_batch.view(1, S).to(dev))  # [1, S, d]
        x = x.to(dtype_w)
        ...
        if cfg.block_group_size == 1:
            blocks = list(self.model.transformer.blocks)
        else:
            blocks = []
            for bg in self.model.transformer.block_groups:
                blocks.extend(list(bg))

        updated_req_ids: Set[int] = set()

        # 2) 레이어 순회
        for layer_idx, block in enumerate(blocks):
            # ---- Stage A: projection (merge)
            if isinstance(block, (LLaDALlamaBlock, LLaDASequentialBlock)):
                x_norm_all = block.attn_norm(x)  # [1, S, d]
                if isinstance(block, LLaDALlamaBlock):
                    q_all = block.q_proj(x_norm_all)
                    k_all = block.k_proj(x_norm_all)
                    v_all = block.v_proj(x_norm_all)
                else:
                    qkv_all = block.att_proj(x_norm_all)
                    q_all, k_all, v_all = qkv_all.split(block.fused_dims, dim=-1)
            else:
                raise RuntimeError(f"Unsupported block type at layer {layer_idx}: {type(block)}")

            # ---- Stage B: attention per-segment (serial)
            seg_spans_for_ff: List[Tuple[int, int]] = []
            for off, Lseg, rid in segments:
                if Lseg <= 0:
                    continue

                qs = q_all[:, off:off + Lseg, :]
                ks = k_all[:, off:off + Lseg, :]
                vs = v_all[:, off:off + Lseg, :]

                layer_past = self._orca_kv_store[rid][layer_idx]
                req_abs_start = int(pos_batch[off].item())

                if layer_past is None:
                    # append-only (첫 prefill or 캐시 없음)
                    att_out, present = block.attention(
                        qs, ks, vs,
                        attention_bias=None,
                        layer_past=None,
                        use_cache=True,
                    ...
                    x[:, off:off + Lseg, :] = x[:, off:off + Lseg, :] + block.dropout(att_out)

                else:
                    L_full = int(layer_past[0].size(-2))
                    if req_abs_start >= L_full:
                        # append-only
                        att_out, present = block.attention(
                            qs, ks, vs,
                            attention_bias=None,
                    ...
                    elif req_abs_start + Lseg <= L_full:
                        # pure replace
                        rp = torch.zeros((1, L_full), dtype=torch.bool, device=dev)
                        rp[:, req_abs_start:req_abs_start + Lseg] = True
                        att_out, present = block.attention(
                            qs, ks, vs,
                            attention_bias=None,
                            layer_past=layer_past,
                        ...
                        # partial: 앞 replace, 뒤 append
                        replace_len = L_full - req_abs_start
                        append_len = Lseg - replace_len
                        # replace
                        rp = torch.zeros((1, L_full), dtype=torch.bool, device=dev)
                        rp[:, req_abs_start:req_abs_start + replace_len] = True
                        att_r, present = block.attention(
                            qs[:, :replace_len, :],
                            ks[:, :replace_len, :],
                            vs[:, :replace_len, :],
                            attention_bias=None,
                        ...
                        x[:, off:off + replace_len, :] = x[:, off:off + replace_len, :] + block.dropout(att_r)
                        # append
                        att_a, present = block.attention(
                            qs[:, replace_len:, :],
                            ks[:, replace_len:, :],
                            vs[:, replace_len:, :],
                            attention_bias=None,
                        ...
                        x[:, off + replace_len:off + Lseg, :] = (
                            x[:, off + replace_len:off + Lseg, :] + block.dropout(att_a)
                        )

                if present is not None:
                    self._orca_kv_store[rid][layer_idx] = present
                    updated_req_ids.add(rid)

                seg_spans_for_ff.append((off, Lseg))

            # ---- Stage C: FFN (merge)
            if seg_spans_for_ff:
                cat_x = torch.cat([x[:, off:off + Lseg, :] for (off, Lseg) in seg_spans_for_ff], dim=1)
                z = block.ff_norm(cat_x)
                if isinstance(block, LLaDALlamaBlock):
                    z1 = block.ff_proj(z)
                    z_up = block.up_proj(z)
                    z_act = block.act(z1)
                    z_out = block.ff_out(z_act * z_up)
                else:
                    z_mid = block.act(block.ff_proj(z))
                    z_out = block.ff_out(z_mid)
                z_out = block.dropout(z_out)

                # scatter back (+residual)
                cursor = 0
                for off, Lseg in seg_spans_for_ff:
                    x[:, off:off + Lseg, :] = x[:, off:off + Lseg, :] + z_out[:, cursor:cursor + Lseg, :]
                    cursor += Lseg

        # 3) 최종 LN
        x = self.model.transformer.ln_f(x)  # [1, ΣL, d]

        # 4) logits
        if cfg.weight_tying:
            logits = F.linear(x, self.model.transformer.wte.weight, None)  # [1, ΣL, V]
        else:
            logits = self.model.transformer.ff_out(x)
        if cfg.scale_logits:
            logits = logits * (1.0 / math.sqrt(cfg.d_model))

        logits_cat = logits.view(-1, logits.size(-1))  # [ΣL, V]

        return {
            "logits_cat": logits_cat,
            "updated_kv_handles": {rid: True for _, _, rid in segments if rid in updated_req_ids},
        }
